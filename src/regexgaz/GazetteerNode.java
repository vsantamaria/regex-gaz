...
/**
 * <p>
 * A node in a gazetteer list allowing an arbitary amount of features to be
 * added as metadata to an entry
 * </p>
 * @author JLy
 *
 */
public class GazetteerNode {
	...
	/**
	 * The features associated to the entry. If there are no features for this
	 * entry, it is null
	 */
	 ...
	 *
	 * @param node the gazetteer node to be parsed
	 * @param separator the separator used in the gazetteer node string to
	 * delimit each name-value pair of features. If the separator is null, then
	 * the whole node will be used as the gazetteer entry
	 */
	 ...
	 * by whatever this GazetteerNode's separator has been set to, convert it to
	 * the equivalent map.
	 *
	...
	private Map<String, Object> getFeatures(String features, boolean isOrdered) {
		...
		ArrayList<String> tempPairs = new ArrayList<>();
	...
	/**
	 * Converts a featureMap to separated name value pairs. Note: the string
	 * will begin with the separator character.
	 *
	...
	public String featureMapToString(Map<String, Object> featureMap) {
		...
		if (featureMap instanceof LinkedHashMap) {
			...
			List<String> sortedKeys = new ArrayList<>(featureMap.keySet());
	...
	public String toString() {
		if (featureMap == null || separator == null) {
			...
			return entry + featureMapToString(featureMap);
		}
	}
	 ...
	 * @return true if the string representation of the entry and weighting
	 * match.
	 */
	@Override
