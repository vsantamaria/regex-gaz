 ...
 */
public class GazetteerList extends AbstractLanguageResource
		implements
		List<GazetteerNode> {
	 ...
	 */
	private List<GazetteerNode> entries = new ArrayList<>();
	...
	public void load(boolean isOrdered) throws ResourceInstantiationException {
		...
		try {
			...
			listReader.close();
		} catch (ResourceInstantiationException | IOException | GateRuntimeException x) {
	...
	public void store() throws ResourceInstantiationException {
		try {
			...
			listWriter.close();
		} catch (ResourceInstantiationException | IllegalArgumentException | IOException x) {
			throw new ResourceInstantiationException(x.getClass() + ":"
					+ x.getMessage());
		}
		isModified = false;
	...
	/**
	 * Gets an iterator over the list. It is not dangerous if the iterator is
	 * modified since there are no dependencies of entries to other members
	 */
	@Override
	 ...
	 * @return true if all the elements where Strings and all are sucessfully
	 * added
	 */
	@Override
	...
	/**
	 * Retrieves the string representation of the gaz list according to its
	 * mode. If {@link #LIST_MODE} then all the entries are dumped sequentially
	 * to a string. If {@link #STRING_MODE} then the content (a string) of the
	 * gaz list is retrieved.
	 *
	...
	public List<GazetteerNode> getEntries(){
		return entries;
	}
